...
# Our Method
thesis_stand <- as.data.frame(scale(df_set3,
                                    center = F,
                                    scale = apply(df_set3, 2, sd, na.rm = TRUE)
))
# root-mean-square standardization
rms_stand <- as.data.frame(scale(df_set3,
                                 center = F,
                                 scale = T))
# N(0,1) then min-max standadization
stand <- as.data.frame(scale(df_set3,
                             center = T,
                             scale = T))
...
samp_ex <- sort(sample(nrow(df_set3), nrow(df_set3) * .8))
# Training data for each standardization
original_train <- df_set3[samp_ex,]
