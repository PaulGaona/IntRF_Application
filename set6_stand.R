...
# Our Method
thesis_stand <- as.data.frame(scale(df_set6,
                                    center = F,
                                    scale = apply(df_set6, 2, sd, na.rm = TRUE)
))
# root-mean-square standardization
rms_stand <- as.data.frame(scale(df_set6,
                                 center = F,
                                 scale = T))
# N(0,1) then min-max standadization
stand <- as.data.frame(scale(df_set6,
                             center = T,
                             scale = T))
...
samp_ex <- sort(sample(nrow(df_set6), nrow(df_set6) * .8))
# Training data for each standardization
original_train <- df_set6[samp_ex,]
