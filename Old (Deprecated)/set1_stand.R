...
# Our Method
thesis_stand <- as.data.frame(scale(df_set1,
                                    center = F,
                                    scale = apply(df_set1, 2, sd, na.rm = TRUE)
))
# root-mean-square standardization
rms_stand <- as.data.frame(scale(df_set1,
                                 center = F,
                                 scale = T))
# N(0,1) then min-max standadization
stand <- as.data.frame(scale(df_set1,
                             center = T,
                             scale = T))
...
samp_ex <- sort(sample(nrow(df_set1), nrow(df_set1) * .8))
# Training data for each standardization
original_train <- df_set1[samp_ex,]
