...
set2_p <- IntRF.thesis::int_plot(
  ...
  ggplot2::scale_x_continuous(
    limits = c(
      min(df_set2$Xc - df_set2$Xr)-5, # Set lower limit of x-axis
      max(df_set2$Xc + df_set2$Xr)+5
    ), # Set upper limit of x-axis
  ...
  ggplot2::scale_y_continuous(
    limits = c(
      min(df_set2$Yc - df_set2$Yr)-1, # Set lower limit of y-axis
...
set3_p <- IntRF.thesis::int_plot(
  ...
  ggplot2::scale_x_continuous(
    limits = c(
      min(df_set3$Xc - df_set3$Xr)-1, # Set lower limit of x-axis
      max(df_set3$Xc + df_set3$Xr)+1
    ), # Set upper limit of x-axis
    n.breaks = 10, # Set number of x-axis breaks to 10
    labels = scales::label_wrap(6) # Wrap x-axis labels to fit within 6 lines
  ) +
  ggplot2::scale_y_continuous(
    limits = c(
      min(df_set3$Yc - df_set3$Yr)-1, # Set lower limit of y-axis
      max(df_set3$Yc + df_set3$Yr)+10 # Set upper limit of y-axis
    ),
    n.breaks = 8 # Set number of y-axis breaks to 8
  )
