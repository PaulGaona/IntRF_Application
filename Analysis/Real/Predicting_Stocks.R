...
# Use the IntRF package to train a random forest model for
# interval regression
set.seed(1)
...
set.seed(1)
# create random forest model for JPM stock price using
# all other variables
...
set.seed(1)
# create random forest model for JPM stock price using all
# other variables
# except actual stock price
...
# Define the stock locations for each stock in the
# dataset
...
# Make predictions using the model on the testing data
ctpred_ba <- predict(irt_ba,
  newdata = price_test_ba,
  type = "matrix"
)[, 1]
rtpred_ba <- predict(irt_ba,
  newdata = price_test_ba,
  type = "matrix"
)[, 2]
...
set.seed(1)
# create CCRM model for the JPM stock price as a
# function of the DJI stock price
...
# combine accuracy metrics from all models into a
# single data frame for comparison
...
# Make predictions using the model on the testing data
ctpred_ge <- predict(irt_ge,
  newdata = price_test_ge,
  type = "matrix"
)[, 1]
rtpred_ge <- predict(irt_ge,
  newdata = price_test_ge,
  type = "matrix"
)[, 2]
...
set.seed(1)
# create random forest model for JPM stock price using
# all other variables except actual stock price
